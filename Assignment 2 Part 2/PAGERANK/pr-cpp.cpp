...
vector<float> pagerankgenerator(vector<vector<int>> incominglinks,vector<int> numlinks,vector<int> leafnodes,double alpha,double convergence,int checksteps){
    ...
    vector<float> pgr_intermediate(n,1.0/n),iold(n,1.0/n);
    bool done = false;
    double diff = 1.0f;
    int i=0;
    while(diff > convergence && i < checksteps){
        ...
        for(int i=0;i<n;i++){
            sum += pgr_intermediate[i];
        }
        
        for(int i=0;i<n;i++){
            pgr_intermediate[i] = pgr_intermediate[i]/sum;
        }

        swap(pgr_intermediate,iold);
        ...
        for(int j=0;j<n;j++){
            ...
            pgr_intermediate[j] = dot*alpha + oneav + oneiv;
        }

        diff = 0;
        for(int i=0;i<n;i++){
            diff += fabs(pgr_intermediate[i]-iold[i]);
        }
        i++;
    }
    return pgr_intermediate;
}
